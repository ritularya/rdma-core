//WORKINGG, not even twice allocation
...
static struct verbs_context *dummy_alloc_context(struct ibv_device *ibdev, int cmd_fd, void *private_data)
{
    ...
    // Manually initialize the ibv_context portion
    ctx->context.device = ibdev;
    ctx->context.cmd_fd = cmd_fd;

    verbs_set_ops(ctx, &dummy_ctx_ops);
